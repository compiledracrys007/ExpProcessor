...
public:
    ...
    std::string get_info() const {
        std::stringstream ss;
        ss << "Matmul Unit ID: " << id 
           << ", with tile sizes M: " << tile_m 
           << ", N: " << tile_n 
           << ", K: " << tile_k;
        return ss.str();
    }
...
public:
    ...
    // Constructor
    ComputeCore(int id, size_t local_memory, const std::vector<MatmulUnit>& matmul_units) 
        : id(id), local_memory(local_memory), matmul_units(matmul_units) {}

    std::string get_info() const {
        std::stringstream ss;
        ss << "Compute Unit ID: " << id 
           << " with Local Memory: " << local_memory << " bytes";
        return ss.str();
    }
};
...
public:
    ...
    // Constructor
    Processor(std::string name, size_t global_memory, const std::vector<ComputeCore>& compute_cores) 
    ...
    std::string get_device_info() const {
        ...
        for (const auto& core : compute_cores) {
            ss << "  " << core.get_info() << "\n";
            for (const auto& mm_unit : core.matmul_units) {
                ss << "    " << mm_unit.get_info() << "\n";
            }
        }
        return ss.str();
